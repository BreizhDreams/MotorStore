...
class ContainRepository extends ServiceEntityRepository
{
    ...
    public function save(Contain $entity, bool $flush = false): void
    {
    ...
    public function findAdvantageInFidelityCard(int $fidelityCardId, int $advantageId) {
        $query = $this
            ->createQueryBuilder('c')
            ->select('c')
            ->andWhere("c.fidelityCardVO = :fidelityCardId")
            ->andWhere("c.advantageVO = :advantageId")
            ->setParameter('fidelityCardId',$fidelityCardId)
            ->setParameter('advantageId',$advantageId);
